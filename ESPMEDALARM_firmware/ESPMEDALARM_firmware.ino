...
#define AP_PASSWORD       ""            // "" = open network. Set a password (>=8 chars) to secure it.
...
struct QueueItem { int medIndex; int timeIndex; };
QueueItem alarmQueue[MAX_QUEUE];
int qHead = 0, qTail = 0, qCount = 0;

struct SkippedEntry { char name[32]; char time[6]; };
...
// STATE MACHINE
// ---------------------------------------------------------------------------
enum SystemState { STATE_WAIT_SYNC, STATE_AP_MODE, STATE_IDLE, STATE_ALARM, STATE_BOX_CLOSING };
...
void handleButton() {
  ...
  if (stable) {
    ...
    if (isPressedNow && buttonPressed) {
      ...
      if (!longPressTriggered && heldFor >= LONG_PRESS_SYNC_MS &&
          (systemState == STATE_WAIT_SYNC || systemState == STATE_IDLE)) {
...
void drawIdle() {
  ...
  if (skippedCount == 0) {
    ...
    for (int i = 0; i < skippedCount && shown < 2; i++, shown++) {
      char line[28];
      snprintf(line, sizeof(line), "%s %s", skippedList[i].time, skippedList[i].name);
